...
def _list_audio_files() -> list[str]:
    ...
    return [
        ...
        and not af.startswith("EXAMPLE_")
        and os.path.splitext(af)[1].lower() in AUDIO_EXTENSIONS
    ]
...
def run_segment() -> None:
    ...
    if not found_any:
        print("No audio files with matching segment definitions found in data/02_segments/.")
...
def _load_substitutions(path: str) -> list[tuple[str, str]]:
    """Load substitution pairs from a file.

    Format (one rule per line):
        original>replacement       # optional inline comment
...
def _apply_substitutions(text: str, subs: list[tuple[str, str]]) -> str:
    """Apply word-boundary substitutions to text, preserving any trailing suffix.
    
    eg. rule IJ>AEGEE will replace:
        IJ -> AEGEE
        IJs -> AEGEEs
        IJ's -> AEGEE's
    """
    for original, replacement in subs:
        pattern = r'\b' + re.escape(original) + r'(\w*)\b'
...
def run_enhance() -> None:
    ...
    subs = _load_substitutions(SUBSTITUTIONS_FILE)
    if subs:
        print(f"  Loaded {len(subs)} substitution(s) from {SUBSTITUTIONS_FILE}")
    else:
        print(f"  No substitutions file at {SUBSTITUTIONS_FILE} — copying raw transcripts as-is")

    if not os.path.isdir(RAW_DIR):
        print("No raw transcripts found. Run 'make transcribe' first.")
        return

    raw_files = sorted(f for f in os.listdir(RAW_DIR) if not f.startswith(".") and f.endswith(".txt"))
    if not raw_files:
    ...
    for rf in raw_files:
        ...
        t0 = time.time()
        final_text = _apply_substitutions(raw_text, subs)
        elapsed = fmt_duration(time.time() - t0)
        replacements_made = len(raw_text) - len(final_text) + sum(
            len(r) - len(o) for o, r in subs
        )
        print(f"  Applied {len(subs)} substitution rule(s) in {elapsed} — {len(final_text):,} chars")
